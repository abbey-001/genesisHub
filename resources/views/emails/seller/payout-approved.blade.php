{{-- resources/views/emails/seller/payout-approved.blade.php --}}
@php
    $sellerName  = $notifiable->name ?? 'Seller';
    $approvedAt  = $payout->processed_at?->format('d M Y, g:i A') ?? now()->format('d M Y, g:i A');
...
<x-emails.layouts.base
    ...
    <p class="greeting">Approved, {{ $sellerName }}!</p>
    <p class="email-tagline">Your payout request has been approved</p>

    <p>
        Your payout request has been reviewed and approved by our team.
    ...
    <div style="background:linear-gradient(135deg,#fef9f0,#fef3e2);border:1px solid #fde68a;border-radius:10px;padding:20px 24px;margin:22px 0;display:table;width:100%;">
        <div style="display:table-cell;vertical-align:middle;width:52px;">
            <div style="width:44px;height:44px;background:#f5c34b;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:20px;">✓</div>
        </div>
    ...
    <div style="background:#f9fafb;border:1px solid #e0e4ea;border-radius:10px;padding:22px;margin:20px 0;text-align:center;">
        ...
        <p style="font-size:12px;color:#9ca3af;margin:0;">via {{ $methodLabel }}</p>
    </div>
    ...
    <div class="alert-box alert-info">
        ...
        a final confirmation email the moment your funds have been sent.
